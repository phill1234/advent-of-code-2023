...
public class CardHandPart2 implements Comparable<CardHandPart2> {
    enum CardValue {
        JOKER('J', 1),  TWO('2', 2), THREE('3', 3), FOUR('4', 4), FIVE('5', 5), SIX('6', 6), SEVEN('7', 7), EIGHT('8', 8), NINE('9', 9), TEN('T', 10), QUEEN('Q', 12), KING('K', 13), ACE('A', 14);
    ...
    public CardHandPart2(String cards, int bid) {
        this.cards = cards;
        this.bid = bid;
    }
    ...
    @Override
    public int compareTo(CardHandPart2 o) {
    ...
    public static boolean isFullHouse(String hand1) {
        for (Character card : hand1.toCharArray()) {
            String handWithTransformedJokers = hand1.replace('J', card);
            int count = StringUtils.countMatches(handWithTransformedJokers, String.valueOf(card));
            if (count == 3) {
                for (Character card2 : hand1.toCharArray()) {
                    if (card2 == card) {
                        continue;
                    }
                    int count2 = StringUtils.countMatches(handWithTransformedJokers, String.valueOf(card2));
                    if (count2 == 2) {
                        return true;
                    }
                }
    ...
    private static boolean isPair(String hand) {
        ...
        for (Character card : hand.toCharArray()) {
            String handWithTransformedJokers = hand.replace('J', card);
            int count = StringUtils.countMatches(handWithTransformedJokers, String.valueOf(card));
            if (count == 2) {
                return true;
            }
